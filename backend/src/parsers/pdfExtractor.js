...
import { clean as normalizeText } from "./parsers/common.js";
...
function mapPasswordError(error) {
  const rawMessage =
    error === null || error === undefined
      ...
      ? error
      : typeof error === "object"
      ? String((error.message !== undefined ? error.message : error))
      : String(error);
  const message = rawMessage.toLowerCase();
  const name = String(error && typeof error === "object" ? (error.name !== undefined ? error.name : "") : "");
...
async function extractPdf(filePath, password) {
  ...
  try {
    ...
    console.error("extractPdf error raw:", error);
    const passwordError = mapPasswordError(error);
    console.error("extractPdf passwordError:", passwordError);
    if (passwordError) {
